...
import { 
  CheckCircle2, AlertTriangle, XCircle, UserCog,
  Target, ArrowDown, Shield, MapPin, Search, Mail
} from 'lucide-react';
...
function DealCard({ deal, result, showMao }: { deal: Deal; result: FlipResult; showMao?: boolean }) {
  return (
...
export default function AgentManagementPage() {
  ...
  // Agent permissions (stored locally for now since we have 1 agent)
  ...
  useEffect(() => {
    async function fetchAgents() {
      try {
        ...
        // We just store user IDs - we can't query auth.users directly
        const agentInfos: AgentInfo[] = (data || []).map(r => ({
          userId: r.user_id,
          email: 'Agent', // We'll show the user_id or a label
        }));
  ...
  // Filter deals created by agents
  const agentDeals = useMemo(() => {
    const agentUserIds = agents.map(a => a.userId);
    return deals.filter(d => d.createdBy && agentUserIds.includes(d.createdBy));
  }, [deals, agents]);

  const categorized = useMemo(() => {
    const activeDeals = agentDeals.filter(d => d.status !== 'not_relevant');

    const withScores = activeDeals
      .map(deal => {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Monitor agent activity • {agentDeals.length} total deals from agents
          </p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Switch 
              checked={permissions.addressSearch} 
              onCheckedChange={(v) => {
                setPermissions(p => ({ ...p, addressSearch: v }));
                toast({ title: `Address Search ${v ? 'enabled' : 'disabled'} for agent` });
              }}
            />
          </div>
          <div className="flex items-center justify-between">
            ...
            <Switch 
              checked={permissions.marketSearch} 
              onCheckedChange={(v) => {
                setPermissions(p => ({ ...p, marketSearch: v }));
                toast({ title: `Market Search ${v ? 'enabled' : 'disabled'} for agent` });
              }}
            />
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <Label>Email Search (Gmail)</Label>
            </div>
            <Switch 
              checked={permissions.emailSearch} 
              onCheckedChange={(v) => {
                setPermissions(p => ({ ...p, emailSearch: v }));
                toast({ title: `Email Search ${v ? 'enabled' : 'disabled'} for agent` });
              }}
            />
          </div>
        </CardContent>
      ...
      <Tabs defaultValue="good">
        <TabsList>
          ...
          <TabsTrigger value="rest" className="gap-1.5">
            Others ({categorized.rest.length})
          </TabsTrigger>
        </TabsList>

        <TabsContent value="good" className="space-y-3 mt-4">
          {categorized.good.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No good deals from agent yet</p>
          ) : (
        ...
        <TabsContent value="potential" className="space-y-3 mt-4">
          {categorized.potential.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No potential deals</p>
          ) : (
            <>
              <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-sm text-amber-200">
                <Target className="w-4 h-4 inline mr-1.5" />
                These deals could be profitable with a price negotiation of less than 8%.
                The <strong>MAO</strong> shows the target offer price for 18% ROI.
              </div>
        ...
        <TabsContent value="rest" className="space-y-3 mt-4">
          {categorized.rest.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No other deals from agent</p>
          ) : (
