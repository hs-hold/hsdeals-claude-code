...
export function useUserState() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setSelectedState(data?.selected_state || null);
      setIsLoading(false);
    };
    load();
  }, [user]);

  const updateState = useCallback(async (stateCode: string) => {
    if (!user) return;
  ...
  return { selectedState, stateName, isLoading, updateState, US_STATES };
}
